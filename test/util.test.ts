...
describe("util functionalities", () => {
	...
	describe("getFunctionMetaData", () => {
		test("no params", () => {
			...
			expect(getFunctionMetaData(content, startIdx)).toEqual({
				parameters: [],
				returnType: "any",
				paramsOpeningBracketIdx: 17,
			});
		});
		test("one param", () => {
			...
			expect(getFunctionMetaData(content, startIdx)).toEqual({
				parameters: [{ context: "variable", name: "someParam", type: "string" }],
				returnType: "any",
				paramsOpeningBracketIdx: 17,
			});
		});
		test("multiple params", () => {
			...
			expect(getFunctionMetaData(content, startIdx)).toEqual({
				parameters: [
					{ context: "variable", name: "someParam", type: "string" },
					{ context: "variable", name: "other", type: "number" },
				],
				returnType: "any",
				paramsOpeningBracketIdx: 17,
			});
		...
		test("object as param", () => {
			...
			expect(getFunctionMetaData(content, startIdx)).toEqual({
				parameters: [{ context: "variable", name: "someParam", type: "{key: string}" }],
				returnType: "any",
				paramsOpeningBracketIdx: 17,
			});
		});
